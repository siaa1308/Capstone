...
def calendar_boundary(dataset: Path, bank: str) -> float:
    ...
    origin = pd.to_datetime(frame.timestamp, format="ISO8601").min()
...
def load_development_clients(dataset: Path) -> dict:
    """Single development loader: the test split is deliberately unavailable."""
    encoders = fit_shared_feature_encoders(dataset, BANKS)
    clients = {}
    for bank in BANKS:
        static, streams, *_ = build_bank_data(dataset, bank, *encoders, splits=("training", "validation"))
        clients[bank] = (static, streams, calendar_boundary(dataset, bank))
    return clients


def fast_choose_threshold(labels: np.ndarray, scores: np.ndarray) -> float:
    """Exactly the original 501-quantile F1 search, using cumulative counts.

    Avoid repeatedly scanning large validation arrays in sklearn for every
    candidate. >= comparisons, candidate quantiles and first-maximum ties match
    the frozen threshold selector; this does not change model selection.
    """
    candidates = np.unique(np.quantile(scores, np.linspace(0, 1, 501)))
    order = np.argsort(scores, kind="stable")
    sorted_scores, sorted_labels = scores[order], labels[order]
    prefix = np.concatenate(([0.], np.cumsum(sorted_labels, dtype=np.float64)))
    indices = np.searchsorted(sorted_scores, candidates, side="left")
    positives = prefix[-1]
    true_positives = positives - prefix[indices]
    denominators = positives + len(labels) - indices
    f1 = np.divide(2 * true_positives, denominators, out=np.zeros_like(denominators, dtype=float), where=denominators != 0)
...
def train_client(model, static, training, boundary, cfg, seed):
    if getattr(cfg, "rng_policy", "per_client") == "per_client":
        set_seed(seed)
...
def validation(model, static, streams, batch_size):
    ...
    threshold = fast_choose_threshold(labels, scores)
...
def parse_args():
    ...
    parser.add_argument("--rng-policy", choices=("per_client", "global"), default="per_client")
    parser.add_argument("--initial-checkpoint", type=Path,
                        help="Optional trusted existing global checkpoint; never loaded from Kafka")
    ...
    if cfg.initial_checkpoint is not None and cfg.mode != "federated":
        parser.error("Global checkpoint initialization is only supported in federated mode")
...
def main():
    ...
    clients = load_development_clients(cfg.dataset_dir)
    ...
    if cfg.initial_checkpoint is not None:
        with torch.serialization.safe_globals([PosixPath]):
            checkpoint = torch.load(cfg.initial_checkpoint, map_location="cpu", weights_only=True)
        global_model.load_state_dict(checkpoint["state_dict"], strict=True)
    ...
    if cfg.initial_checkpoint is not None:
        metadata["initial_checkpoint_sha256"] = hashlib.sha256(cfg.initial_checkpoint.read_bytes()).hexdigest()
    json_write(cfg.output_dir / "manifest.json", metadata)
    (cfg.output_dir / "source_snapshot.py").write_bytes(Path(__file__).read_bytes())
    ...
    if cfg.mode == "local":
        ...
        if cfg.initial_checkpoint is not None:
            best_rows = [{"bank": bank, **validation(global_model, static, streams, cfg.batch_size)}
                         for bank, (static, streams, _) in clients.items()]
            best_score = float(np.mean([row["validation"]["pr_auc"] for row in best_rows]))
            history.append({"round": 0, "macro_validation_pr_auc": best_score, "per_bank": best_rows,
                            "initial_checkpoint": True, "elapsed_seconds": time.monotonic() - started})
            torch.save({"state_dict": global_model.state_dict(), "args": configuration, "best_round": 0},
                       cfg.output_dir / "global_model.pt")
            print(f"INITIAL validation={best_score:.6f}", flush=True)
